...
def map_reac_and_frag(reac_mols: List[Chem.Mol], frag_mols: List[Chem.Mol]):
    ...
    new_reac = [Chem.Mol(reac_mols[max_overlaps[i]]) for i in max_overlaps]
...
def correct_mol_1(mol, is_nitrine_c):
    ...
    for atom in mol.GetAtoms():
        ...
        if (
            ...
            and sum([i.GetBondTypeAsDouble() for i in atom.GetBonds()]) == 4
            and 1.5 not in [i.GetBondTypeAsDouble() for i in atom.GetBonds()]
                ...
                for reac_edit in reac_edits:
                    ...
                    if e in reac_frag_map_num_lis and b in frag_1_map_num_lis:
                        ...
                        if len(attach_map_num_1) == 1:
                            ...
                            if [str(attach_map_num_1[0]), str(atom.GetAtomMapNum() - 500)] in [
                                i.split(":")[:2] for i in reac_edits
                            ]:  # 上一个合成子上的连接点和本离去基团的连接点配对
    ...
    for chai_edit in chai_edits:
        ...
        if chai_edit[-2:] == ":R":
            ...
            if temp_mol_dic[atom.GetIdx()] == "S":
                ...
                atom.SetChiralTag(Chem.ChiralType.CHI_TETRAHEDRAL_CW)
                temp_mol_dic = dict(
                    Chem.FindMolChiralCenters(Chem.MolFromMolBlock(Chem.MolToMolBlock(prod_mol)), includeUnassigned=True)
                )
        ...
        if lg_smile not in ["-1", "1", "2"]:
            ...
            for idx in range(len(map_nums)):
                ...
                if atom.GetSymbol() == "O" and atom.GetTotalValence() == 0 and atom.GetFormalCharge() == 0 and is_multi_bond == 0:
                    ...
                    atom.GetSymbol() == "S" and atom.GetTotalValence() == 1 and atom.GetFormalCharge() == 1 and is_multi_bond == 0
                ):
                    bond_float = 2.0
                elif (
                    atom.GetSymbol() == "P" and atom.GetTotalValence() == 3 and atom.GetFormalCharge() == 0 and is_multi_bond == 0
                ):
                    bond_float = 2.0
                elif (
                    atom.GetSymbol() == "C" and atom.GetTotalValence() == 2 and atom.GetFormalCharge() == 0 and is_multi_bond == 0
                ):
                    bond_float = 2.0
                elif (
                    atom.GetSymbol() == "N" and atom.GetTotalValence() == 2 and atom.GetFormalCharge() == 1 and is_multi_bond == 0
                ):
                    bond_float = 2.0
                elif (
                    atom.GetSymbol() == "N" and atom.GetTotalValence() == 1 and atom.GetFormalCharge() == 0 and is_multi_bond == 0
                ):
                    bond_float = 2.0
                elif (
                    atom.GetSymbol() == "N"
                    ...
                    atom.GetSymbol() == "O" and atom.GetTotalValence() == 1 and atom.GetFormalCharge() == 1 and is_multi_bond == 0
                ):
                    bond_float = 2.0
                elif (
                    atom.GetSymbol() == "N" and atom.GetTotalValence() == 0 and atom.GetFormalCharge() == 0 and is_multi_bond == 0
                ):
                    bond_float = 3.0
                elif (
                    atom.GetSymbol() == "C" and atom.GetTotalValence() == 1 and atom.GetFormalCharge() == 0 and is_multi_bond == 0
                ):
                    bond_float = 3.0
                elif (
                    atom.GetSymbol() == "C"
